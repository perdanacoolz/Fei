...
import CreateEditRegion from './CreateEditRegion';
...
const ListRegion = () => {
  ...
  const toggleEdit = (id) => {
    ApiService.getDataById(param, id).then((res) => {
      if (res !== null) {
        ...
        setcontinentId(res.data.continentId);
      }
      setId(id);
      setIsModal(true);
      setTitleModal('Edit');
  ...
  return (
    <>
      ...
      <CreateEditRegion isModal={isModal} handleClick={handleClick} titleModal={titleModal} title={title} param={param} id={id}
        masterCode={masterCode} abbreviation={abbreviation} name={name} continentId={continentId}
      />
    </>
